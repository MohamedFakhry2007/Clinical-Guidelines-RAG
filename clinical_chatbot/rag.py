...
from clinical_chatbot.database import get_all_policy_documents, log_chat_interaction
...
def initialize_retrieval_system():
    ...
    logger.info("Initializing Clinical Retrieval System")
    
    try:
        # Get all policy documents from the database
        documents = get_all_policy_documents() # Retrieving Guidelines
...
async def generate_response(query, relevant_docs):
    ...
    try:
        ...
        prompt = f"""
        You are an expert Clinical AI Assistant for a hospital. Your task is to answer medical queries based STRICTLY on the provided clinical protocols.
        
        Guidelines:
        1. Use ONLY the Context provided below. Do not use outside medical knowledge.
        2. If the answer is not in the context, say: "عذراً، هذا الإجراء غير موجود في البروتوكولات المعتمدة لدينا." (Sorry, this procedure is not in our approved protocols).
        3. Be direct, professional, and use bullet points for steps.
        4. Always maintain the medical terminology used in the context.
        
        Context (Approved Protocols):
        {context}
        
        Doctor/Nurse Query: {query}
